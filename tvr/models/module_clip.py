...
from .tokenization_clip import SimpleTokenizer as _Tokenizer
...
def tokenize(texts, context_length: int = 77, truncate: bool = False) -> Union[torch.IntTensor, torch.LongTensor]:
    """
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text, return_hidden=False, mask=None, text_vec=None, token_type=False):
        if token_type:
    ...
    def encode_text_img(self, text, img_tokens, text_mask, return_hidden=False, mask=None):
    ...
    def img2text(self, text, image_token, return_hidden=False, mask=None, text_vec=None):
        ...
        # x = hidden[torch.arange(hidden.shape[0]), text.argmax(dim=-1)]
        x = hidden[torch.arange(hidden.shape[0]), pseudo_mask.sum(-1).type(text.dtype)-1]
    ...
    def img4text(self, text, return_hidden=False, mask=None):
        ...
        collect_ind = text == self.vocab_size-1
        collect_ind = collect_ind.nonzero()[:, 1]
        pseudo_mask = torch.ones(image_token.size()[0],image_token.size()[1]).to(mask.device)
        x = torch.cat([x[:, :collect_ind[0]], image_token, x[:, collect_ind[0]:-1]], dim=1).type(self.dtype)
        pseudo_mask = torch.cat([mask[:, :collect_ind[0]], pseudo_mask, mask[:, collect_ind[0]:-1]], dim=1)
        pos_emd = self.positional_embedding[:x.size(1), :].type(self.dtype)
