...
def get_args(description='Disentangled Representation Learning for Text-Video Retrieval'):
    ...
    ## for embedding mode
    parser.add_argument('--embd_mode', type=str, choices=['emcl', 'xpool', 'clip4clip', 'wti'])
    parser.add_argument('--temp_loss_weight', type=float, default=1.0)
    parser.add_argument('--rec_loss_weight', type=float, default=1.0)
    parser.add_argument('--ret_weight', type=float, default=1.0)
    parser.add_argument('--pseudo_ret_weight', type=float, default=1.0)
    ...
    parser.add_argument('--negative_scale', type=float, default=1/2000)
    parser.add_argument('--shift', type=float, default=4) # pre 0.1
    ...
    parser.add_argument('--interact_mode', type=str, default='TI')
...
def save_model(epoch, args, model, type_name=""):
    ...
    torch.save(model_to_save.state_dict(), join(args.output_dir,"best.bin"))
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer,
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        for k, v in loss_dict.items():
            # meters.meters[k].update(reduce_loss(v, args))
            meters.meters[k].update(v)
    ...
    with torch.no_grad():
        ...
        if multi_sentence_:  # multi-sentences retrieval means: one clip has two or more descriptions.
            ...
            for batch in tqdm(test_dataloader):
                ...
                b, *_t = video.shape
                text_feat, cls, text_mask = model.get_text_feat(text_ids, text_mask)
                ids_t.append(inds)
                batch_mask_t.append(text_mask)
                batch_feat_t.append(text_feat)
                batch_cls.append(cls)

                video_feat, video_mask= model.get_video_feat(video, video_mask)
